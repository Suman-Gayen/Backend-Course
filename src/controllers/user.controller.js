...
const userResister = asyncHandler(async (req, res) => {
  const { userName, email, password, fulName, avatar } = req.body;
  ...
  // ----------------/or/---------------------------
  if (
    [userName, email, password, fulName, avatar].some((field=>{   //some() method is used to check if any of the fields are empty or null. If any of the fields are empty or null, then the some() method will return true. If all the fields are not empty or null, then the some() method will return false.
      field?.trim() === ""; //  trim() method is used to remove any whitespace from the beginning and end of the string. If the field is empty or null, then the trim() method will return an empty string. If the field is not empty or null, then the trim() method will return the field itself. 
    }))
  ) {
  ...
  const userExist = User.findOne({ // The findOne() method is used to find a single document in a collection.
    $or: [{ userName }, { email }]  // The $or operator is used to find documents that match any of the conditions specified in the array. 
  })
  ...
  if (!avatarCloudinary || !coverImgCloudinary) {
    throw new ApiError(400, "Please provide all the required fields");
  }
